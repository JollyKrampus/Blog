...
namespace BlazorBlogs
{
    public class Startup
    {
        public Startup(IConfiguration configuration, IWebHostEnvironment env)
        {
            ...
            if (File.Exists(env.ContentRootPath + @"\Upgrade\BlazorBlogsLibrary.dll"))
            {
                var WebConfigOrginalFileNameAndPath = env.ContentRootPath + @"\Web.config";
                var WebConfigTempFileNameAndPath = env.ContentRootPath + @"\Web.config.txt";

                if (File.Exists(WebConfigOrginalFileNameAndPath))
                {
                    // Temporarily rename the web.config file
                    // to release the locks on any assemblies
                    File.Copy(WebConfigOrginalFileNameAndPath, WebConfigTempFileNameAndPath);
                    File.Delete(WebConfigOrginalFileNameAndPath);
                    ...
                    File.Copy(WebConfigTempFileNameAndPath, WebConfigOrginalFileNameAndPath);
                    File.Delete(WebConfigTempFileNameAndPath);
                }

                // Delete current 
                File.Delete(env.ContentRootPath + @"\CustomModules\BlazorBlogsLibrary.dll");
                File.Delete(env.ContentRootPath + @"\CustomModules\BlazorBlogsLibrary.Views.dll");
                ...
                File.Delete(env.ContentRootPath + @"\Upgrade\BlazorBlogsLibrary.dll");
                File.Delete(env.ContentRootPath + @"\Upgrade\BlazorBlogsLibrary.Views.dll");
            }
        }
