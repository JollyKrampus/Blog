...
namespace BlazorBlogsLibrary.Controllers
{
    ...
    public class RSSFeed : Controller
    {
        ...
        public async Task<IActionResult> RssAsync()
        {
            ...
            var feed = new SyndicationFeed(objGeneralSettings.ApplicationName, objGeneralSettings.ApplicationName,
                new Uri(GetBaseUrl()), "RSSUrl", DateTime.Now);
            ...
            {
                var BlogURL = $"{GetBaseUrl()}/ViewBlogPost/{item.BlogId}";
                ...
                items.Add(new SyndicationItem(title, description, new Uri(BlogURL), item.BlogId.ToString(),
                    item.BlogDate));
            }
