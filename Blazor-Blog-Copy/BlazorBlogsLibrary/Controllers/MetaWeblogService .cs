...
namespace BlazorBlogsLibrary.Controllers
{
    public class MetaWeblogService : IMetaWeblogProvider
    {
        ...
        {
            var colBlogInfo = new BlogInfo[1];
        ...
        public async Task<Post[]> GetRecentPostsAsync(string blogid, string username, string password,
            int numberOfPosts)
        {
        ...
        public async Task<string> AddPostAsync(string blogid, string username, string password, Post post, bool publish)
        {
            ...
            if (await IsValidMetaWeblogUserAsync(username, password))
                try
                {
                    ...
                    {
                        var strSummary = ConvertToText(post.description);
                        var intSummaryLength = strSummary.Length;
                        if (intSummaryLength > 500) intSummaryLength = 500;
        ...
        public async Task<bool> DeletePostAsync(string key, string postid, string username, string password,
            bool publish)
        {
            if (await IsValidMetaWeblogUserAsync(username, password))
            {
        ...
        {
            var colCategoryInfo = new List<CategoryInfo>();
        ...
        {
            var mediaInfo = new MediaObjectInfo();
            ...
            if (await IsValidMetaWeblogUserAsync(username, password))
            {
                ...
                mediaInfo.url =
                    $@"{GetBaseUrl()}/blogs/{blogid}/{Path.GetDirectoryName(mediaObject.name).Replace("\\", @"/")}/{fileName}";
            }
            else
            {
                throw new Exception("Bad user name or password");
