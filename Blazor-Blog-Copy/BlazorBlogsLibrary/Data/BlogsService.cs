...
namespace BlazorBlogsLibrary.Data
{
    public class BlogsService
    {
        ...
        {
            page = page - 1;
            var objBlogsPaged = new BlogsPaged();
        ...
        {
            var objBlog = (from blog in _context.Blogs
            ...
            foreach (var item in BlogCategories) objBlog.BlogCategory.Add(item);
            ...
            objBlog.DisqusEnabled =
                Convert.ToBoolean(Settings.FirstOrDefault(x => x.SettingName == "DisqusEnabled").SettingValue);

            if (Settings.FirstOrDefault(x => x.SettingName == "GoogleTrackingID") != null)
                objBlog.GoogleTrackingID =
                    Convert.ToString(Settings.FirstOrDefault(x => x.SettingName == "GoogleTrackingID").SettingValue);
            else
                objBlog.GoogleTrackingID = "";
        ...
        {
            page = page - 1;
            var objCategorysPaged = new CategorysPaged();
        ...
        public Task<bool> CreateCategoryAsync(CategoryDTO objCategoryDTO)
        {
            try
            {
                var objCategorys = new Categorys();
        ...
        public Task<bool> UpdateCategoryAsync(CategoryDTO objCategoryDTO)
        {
            try
            {
                var intCategoryId = Convert.ToInt32(objCategoryDTO.CategoryId);
        ...
        {
            page = page - 1;
            var objFilesPaged = new FilesPaged();
        ...
        public Task<bool> UpdateFilesAsync(FilesDTO objFilesDTO)
        {
            try
            {
                var intFilesId = Convert.ToInt32(objFilesDTO.FileId);
        ...
        public Task<bool> DeleteFilesAsync(FilesDTO objFilesDTO)
        {
            ...
            if (ExistingFiles != null)
            {
                ...
                // Delete the file
                var objFileController = new FileController(_environment);
        ...
        public Task<ExternalConnections> CreateExternalConnectionsAsync(
            ExternalConnectionsDTO objExternalConnectionsDTO)
        {
            try
            {
                var objExternalConnections = new ExternalConnections();
        ...
        public Task<bool> UpdateExternalConnectionsAsync(ExternalConnectionsDTO objExternalConnectionsDTO)
        {
            try
            {
                var intExternalConnectionsId = Convert.ToInt32(objExternalConnectionsDTO.Id);
        ...
        {
            page = page - 1;
            var objLogsPaged = new LogsPaged();
        ...
        {
            page = page - 1;
            var objApplicationUserPaged = new ApplicationUserPaged();
            ...
            foreach (var item in users)
            {
                ...
                objApplicationUser.NewsletterSubscriber =
                    item.NewsletterSubscriber.HasValue ? item.NewsletterSubscriber.Value : false;
        ...
        public async Task<bool> AdminExistsAsync(string strDefaultConnection)
        {
            ...
            using (var context = new BlazorBlogsContext(optionsBuilder.Options))
            {
                ...
                if (AdminRoleList.Count == 0) return false;
        ...
        {
            var colBlogCategory = new List<BlogCategory>();

            foreach (var item in blogCatagories)
            {
                var intBlogCatagoryId = Convert.ToInt32(item);
